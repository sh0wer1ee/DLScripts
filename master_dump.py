...
def retrieveWallData():
    wallPrefix = '21601'
    wallHPData = {}
    enemyparamJson = json.load(
        open(jsonPath + 'EnemyParam.json', encoding='utf8'))
    # 21601*0**  element(1-5) & level(01-99)
    for eid in enemyparamJson:
        if not eid.startswith(wallPrefix):
            continue
...
def retrievePossible70mc():
    # sooooooooo UGLY!
    skilldataJson = json.load(
        open(jsonPath + 'SkillData.json', encoding='utf8'))
    textlabelJson = json.load(
        open(jsonPath + 'TextLabel.json', encoding='utf8'))
    textlabel = {}
    ...
    for sid in skilldataJson:
        if skilldataJson[sid]['_AdvancedSkillLv1'] != 0:
            try:
                ...
                eawid = len(
                    i) + sum(1 for v in i if unicodedata.east_asian_width(v) in 'FWA')
                pad = ' ' * (30 - eawid)
                print("%s%s%s" % (i, pad, v))
            except KeyError:
                try:
